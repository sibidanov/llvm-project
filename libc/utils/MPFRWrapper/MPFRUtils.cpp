...
#define LIBC_TYPES_HAS_DISTINCT_FLOAT128
extern "C" {
...
#if defined(LIBC_TYPES_HAS_DISTINCT_FLOAT128)
template <> struct ExtraPrecision<float128> {
  static constexpr unsigned int VALUE = 512;
};
#endif // LIBC_TYPES_HAS_DISTINCT_FLOAT128
...
public:
  ...
  template <typename XType,
            cpp::enable_if_t<cpp::is_same_v<float128, XType> &&
                                 !cpp::is_same_v<float128, long double>,
                             int> = 0>
  explicit MPFRNumber(XType x,
                      unsigned int precision = ExtraPrecision<XType>::VALUE,
                      RoundingMode rounding = RoundingMode::Nearest)
...
#ifdef LIBC_TYPES_HAS_DISTINCT_FLOAT128
